...
class SpotifyClient:
    ...
    def get_top_artist(self, artist_query):
        ...
        results = artists_results['artists']['items']
        top_result = None
        if len(results) != 0:
    ...
    def get_artist_top_songs(self, artist_query, limit=5):
        ...
        tracks_details_full = self.execute_query(artist_query, 'track', 50)
        tracks_details_canonical = []
        if len(tracks_details_full['tracks']['items']) > 0:
    ...
    def get_artist_genres(self, artist_query):
        ...
        '''
        # Fetch the artists tracks.
        artist_details = self.execute_query(artist_query, 'artist')

        genres = []
        if len(artist_details['artists']['items']) > 0:
